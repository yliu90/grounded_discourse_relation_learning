...
class TemporalCluesNetwork(object):
    ...
    def __init__(self, config):
        ...
        self.temporal_feat_ = tf.concat([self.inner_feat_2, self.temporal_clues, self.diff_feat_A, self.diff_feat_B], axis=1)

        self.W_3, self.b_3 = self._fc_variable([self.inner_dim*1+self.temporal_dim+(self.inner_dim/2)*2, self.rel_dim])
        logit = tf.matmul(self.temporal_feat_, self.W_3) + self.b_3
...
def process():
    ...
    eA = "adam+attacks+sand+with+diamond_axe"
    eB = "adam+gains+1+sand+inventory"
    ...
    for d in datas:
        ...
        eA = items[3]
        eB = items[4]
        eA_idx = event_to_index[eA]
        eB_idx = event_to_index[eB]

        if items[0] == "Cause" and f_mtrx[eA_idx][eB_idx] == 1:
            continue
        if items[0] == "Cause" and p_mtrx[eB_idx][eA_idx] == 1:
            continue

        if items[0] == "Condition" and p_mtrx[eB_idx][eA_idx] == 1:
            continue

        if items[0] == "Temporal_syn" and m_mtrx[eB_idx][eA_idx] != 0 and m_mtrx[eA_idx][eB_idx] != 0:
            continue
        print '+++++++++++++++++++++++++++++++++++++++++++++++++'
        print '+++++++++++++++++++++++++++++++++++++++++++++++++'
        print '+++++++++++++++++++++++++++++++++++++++++++++++++'
        ...
        count += 1
        print count
        print 'Relation : ' , items[0]
        print "eA : " , eA
        print "eB : " , eB
        print '##############################'
        print 'following eA -> eB : ' , f_mtrx[eA_idx][eB_idx]
        print 'previous eA -> eB : ' , p_mtrx[eA_idx][eB_idx]
        print 'meanwhile eA -> eB : ' , m_mtrx[eA_idx][eB_idx]
        
        print '##############################'
        print 'following eB -> eA : ' , f_mtrx[eB_idx][eA_idx]
        print 'previous eB  -> eA : ' , p_mtrx[eB_idx][eA_idx]
        print 'meanwhile eB -> eA : ' , m_mtrx[eB_idx][eA_idx]
